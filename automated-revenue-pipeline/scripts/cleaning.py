...
def clean_sales_df(df: pd.DataFrame) -> pd.DataFrame:
    working = df.copy()
    missing_cols = [c for c in EXPECTED_COLUMNS if c not in working.columns]
    if missing_cols:
    ...
    return working.sort_values(["Date", "Order_ID"], kind="stable").reset_index(drop=True)
...
def main():
    parser = argparse.ArgumentParser(description="Clean messy retail sales dataset.")
    parser.add_argument("--input", default="data/raw_sales.csv")
    parser.add_argument("--output", default="data/cleaned_sales.csv")
